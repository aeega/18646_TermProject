 ...
 *                3. Updated the input& output datatypes at each stage to <float> instead of 
 *                      <uchar> so the performance comparison is accurate.
 *                4. Display images after each stage instead of a single final output after processing 
 *                      all CED stages
 */
...
void CannyEdgeDetector::detect() {
	...
	for (int i = 0; i < rowBlocks; i++) {
		for (int j = 0; j < colBlocks; j++) {
			...
			float gbValues[3][3] = { { 0 } }; // Gaussian blur resulting float values
    ...
    //DBG_PRINTS
    //printf("Number of iterations = %d\n; Total time taken = %f\n avg exec time per iteration = %f\n", counter, (float)total_time, (float)(total_time/counter));
    ...
    //printf("Number of iterations = %d\n; Total time taken = %f\n avg exec time per iteration = %f\n", counter, (float)total_time2, (float)(total_time2/counter));
